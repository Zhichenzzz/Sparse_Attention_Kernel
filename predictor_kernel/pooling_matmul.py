...
for mode in ["fwd"]:
    configs.append(
        triton.testing.Benchmark(
            ...
            styles=[("red", "-"), ("blue", "-")],
            ylabel="ms",
            plot_name=f"mask_predictor-batch{BATCH}-hidden{HIDDEN_DIM}-Seq{N_CTX}-{mode}",
            args={
                "H": N_HEADS,
                "BATCH": BATCH,
                "HEAD_DIM": HEAD_DIM,
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, HIDDEN_DIM, topk, mode, provider, device="cuda"):
    assert mode in ["fwd"]
    warmup = 25
    rep = 100
    dtype = torch.float16
