...
class pooling_mm(torch.autograd.Function):
    ...
    def forward(ctx, q, k, w):
        ...
        qw = torch.empty((BATCH, N_HEADS, N_CTX // BLOCK_M, HIDDEN_DIM), device=q.device, dtype=q.dtype)
        extra_kern_args = {}
...
@triton.jit
def max_pooling(Q, K, Q_down, K_down, n_rep, w, qw,
                stride_qz, stride_qh, stride_qm, stride_qk,  #
                ...
                stride_wh, stride_wk, stride_wn,  #
                stride_qwz, stride_qwh, stride_qwm, stride_qwn,  #
                H, N_CTX,  #
                HEAD_DIM: tl.constexpr,  #
                HIDDEN_DIM: tl.constexpr,  #
                BLOCK_M: tl.constexpr,  #
    ...
    w_offset = off_h.to(tl.int64) * stride_wh
    qw_offset = off_z.to(tl.int64) * stride_qwz + off_h.to(tl.int64) * stride_qwh
    # block pointers
    Q_block_ptr = tl.make_block_ptr(
        base=Q + q_offset,
    ...
    W_block_ptr = tl.make_block_ptr(
        ...
        strides=(stride_wk, stride_wn),
        offsets=(0, 0),
        block_shape=(HEAD_DIM, HIDDEN_DIM),
        order=(1, 0),
    )
    QW_block_ptr = tl.make_block_ptr(
        base=qw + qw_offset,
        shape=(n_rep, HIDDEN_DIM),
        strides=(stride_qwm, stride_qwn),
        offsets=(start_m, 0),
    ...
    qw = tl.sum(q[:, :, None] * w, 1)
    tl.store(QW_block_ptr, qw.to(QW_block_ptr.type.element_ty))
