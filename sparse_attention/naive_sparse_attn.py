...
@triton.jit
def _attn_fwd_inner(acc, l_i, m_i, q, lut,  #
...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    # if STAGE & 1:
    acc = _attn_fwd_inner(acc, l_i, m_i, q, lut,  #
                                    K_block_ptr, V_block_ptr,  #
                                    start_m, qk_scale,  #
                                    TOPK,
    ...
    for causal in [True, False]:
        ...
        configs.append(
            triton.testing.Benchmark(
                ...
                ylabel="ms",
                plot_name=f"sparse-attention-batch{BATCH}-head{N_HEADS}-d{HEAD_DIM}-{mode}-causal={causal}",
                args={
                    "H": N_HEADS,
                    "BATCH": BATCH,
                    "HEAD_DIM": HEAD_DIM,
...
def bench_flash_attention_gqa(BATCH, H, N_CTX, HEAD_DIM, topk, causal, mode, provider, device="cuda"):
    assert mode in ["fwd"]
    warmup = 25
    rep = 100
    dtype = torch.float16
    if "triton" in provider:
        ...
        sm_scale = 1.3
        M = torch.tril(torch.ones((N_CTX, N_CTX), device="cuda"))
        p = torch.randn((BATCH, H, N_CTX, N_CTX), dtype=dtype, device=device)
        if causal:
            p[:, :, M == 0] = float("-inf")
        mxp = torch.max_pool2d(p, (64, 64), stride=(64, 64))
        lut = torch.topk(mxp, topk, dim=-1).indices
...
if __name__ == "__main__":
    ...
    bench_flash_attention_gqa.run(save_path="./naive_results/", print_data=True)
