    ...
    for causal in [True]:
        ...
        configs.append(
            triton.testing.Benchmark(
                ...
                ylabel="ms",
                plot_name=f"sparse-attention-batch{BATCH}-seq{N_CTX}-causal={causal}",
                args={
                    "H": N_HEADS,
                    "BATCH": BATCH,
                    "HEAD_DIM": HEAD_DIM,
...
def bench_flash_attention_gqa(BATCH, H, N_CTX, HEAD_DIM, nz, BLOCK_SIZE, causal, mode, provider, device="cuda"):
    assert mode in ["fwd"]
    warmup = 25
    rep = 100
    dtype = torch.float16
    if "triton" in provider:
        ...
        topk = int(nz * N_CTX / BLOCK_SIZE)
        if causal:
            p[:, :, M == 0] = float("-inf")
            topk = int(N_CTX * solve_quadratic(1, -2, nz) / BLOCK_SIZE)
